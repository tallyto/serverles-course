...
module.exports.hello = async (event) => {
  ...
  const heroes = await dynamodb.scan({ TableName: process.env.HEROES_TABLE }).promise();
  const skills = await dynamodb.scan({ TableName: process.env.SKILLS_TABLE }).promise();
  return {
    statusCode: 200,
    body: JSON.stringify(
      {
